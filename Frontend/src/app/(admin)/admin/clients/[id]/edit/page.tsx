      ...
      <header className="flex justify-between items-start mb-12">
        ...
        <div className="flex gap-4">
          ...
          {user?.isBlocked && (
            <button
              onClick={async () => {
                if (!currentUser) return alert("Brak uprawnień");
                try {
                  const res = await fetch(`${API}/user/${userId}/unblock`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                  ...
                  // odśwież dane
                  const refreshed = await fetch(`${API}/user/${userId}`);
              ...
              className="bg-emerald-600 hover:bg-emerald-700 px-6 py-2 rounded-full font-bold text-[10px] text-white uppercase tracking-widest transition-all ml-4"
            >
              Odblokuj konto
            </button>
          )}
